...
public abstract unsafe class AudioStreamDecoder : IMedia
{
    int error;
    byte* bufferPtr;
    IntPtr audioBuffer;
    ...
    public MediaVolumeState MediaVolumeState { get; private set; }
    ...
    public bool InitDecodecAudio(string path)
    {
        try
        {
            ...
            MediaVolumeState = MediaVolumeState.NotMuted;
            return true;
        }
        catch (Exception ex)
        {
    ...
    public bool SeekProgress(double seekTime)
    {
        try
        {
            ...
            lock (syncLock)
            {
                IsPlaying = false;
                clock.Stop();
                var timestamp = ffmpeg.av_rescale_q((long)(seekTime * ffmpeg.AV_TIME_BASE), ffmpeg.av_get_time_base_q(), audioStream->time_base);
    ...
    byte[] FrameConvertBytes(AVFrame* sourceFrame)
    {
        try
        {
            ...
            if (outputSamplesPerChannel < 0)
                return null;
            var bytes = new byte[outPutBufferLength];
    ...
    public void Mute()
    {
        if (MediaVolumeState == MediaVolumeState.Muted)
            return;

        MediaVolumeState = MediaVolumeState.Muted;
        MuteCore();
    }
    
    public void UnMute()
    {
        if (MediaVolumeState == MediaVolumeState.NotMuted)
            return;
    ...
    public bool IsMuted => MediaVolumeState == MediaVolumeState.Muted;
    ...
    public abstract void UnPauseCore();
    public abstract void MuteCore();
    public abstract void UnMuteCore();
    public abstract void SetVolumeCore(double volume);

    public abstract double GetVolumeCore();
